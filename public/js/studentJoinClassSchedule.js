                ...
                success: function (response) {
                    if (response.status == 404) {
                        ...
                        var strtTime = new Date(
                            "0000, 1, 1" + " " + response.classList.startTime
                        );
                        var ndTime = new Date(
                            "0000, 1, 1" + " " + response.classList.endTime
                        );

                        startTime = strtTime.toLocaleTimeString("en-US", {
                            hour: "numeric",
                            minute: "numeric",
                            hour12: true,
                        });
                        endTime = ndTime.toLocaleTimeString("en-US", {
                            hour: "numeric",
                            minute: "numeric",
                            hour12: true,
                        });

                        $("#instFirstNameAndLastName").val(
                            response.classList.instFirstName +
                                " " +
                                response.classList.instLastName
                        );
                        $("#classID").val(response.classList.classID);
                        $("#program").val(response.classList.program);
                        $("#yearAndSection").val(
                            response.classList.year + response.classList.section
                        );
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == 400) {
                    ...
                    $.each(
                        response.errors.enrollmentKey,
                        function (key, err_value) {
                            $("#enrollmentKeyError").append(
                                "<li>" + err_value + "</li>"
                            );
                        }
                    );
